import { Bot, Context, session, SessionFlavor, InlineKeyboard, StorageAdapter, NextFunction } from "grammy";
import { type Conversation, type ConversationFlavor, conversations, createConversation } from "@grammyjs/conversations";
import { Redis } from "@upstash/redis";
import { prisma } from '@bitremit/database';
import jwt from 'jsonwebtoken';
import dotenv from 'dotenv';
import { buildSendSummary, estimateFiatAmount, validatePhoneNumber } from '../shared/flows.js';
...
class RedisAdapter implements StorageAdapter<any> {
...
async function getConnectedUser(telegramChatId: string) {
...
async function sendFlow(conversation: MyConversation, ctx: MyContext) {
    ...
    if (btnCtx.callbackQuery.data === "recip_new") {
        ...
        const existing = recipients.find((r: any) => r.id === recipientId);
        if (existing) {
...
bot.command("balance", async (ctx: MyContext) => {
    ...
    if (!user) return ctx.reply("Connect your wallet first: https://bitremit.vercel.app");
...
bot.command("status", async (ctx: MyContext) => {
    ...
    if (!params) return ctx.reply("Please provide a reference ID. E.g. '/status 123'");
    
    const user = await getConnectedUser(ctx.chat!.id.toString());
    if (!user) return ctx.reply("Connect your wallet first.");
...
bot.command("send", async (ctx: MyContext) => {
