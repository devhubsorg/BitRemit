...
if (!twilioClient) {
  console.warn(
    "[Worker] Twilio SID missing. SMS notifications will be LOGGED to console but not sent.",
  );
}

interface OfframpJobData {
  txId: string;
  senderAddress: string;
  recipientAddress: string;
  amount: string;
  railType: "MPESA" | "GCASH" | "MTNMOMO";
  recipientPhone: string;
}

async function processOfframpJob(job: Job<OfframpJobData>) {
  const { txId, railType, amount } = job.data;
  let { recipientPhone } = job.data;

  console.log(
    `[Worker] Started processing job ${job.id} for transaction ${txId}`,
  );
  ...
  try {
    if (railType === "MPESA") {
      ...
      const response = await fetch(
        `${process.env.GCASH_MOCK_URL}/v1/payments`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      const response = await fetch(
        `${process.env.MTNMOMO_MOCK_URL}/v1_0/transfer`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    if (success) {
      ...
      const senderDisplay =
        transaction.sender.address.slice(0, 6) +
        "..." +
      ...
      try {
        if (twilioClient) {
          ...
          console.log(
            `[SIMULATED SMS] TO: ${recipientPhone} | MSG: ${smsMessage}`,
          );
        }
      } catch (smsError) {
...
let offrampWorker: Worker<OfframpJobData> | null = null;

function createWorker(): Worker<OfframpJobData> {
  const worker = new Worker<OfframpJobData>(
    "process-offramp",
    processOfframpJob,
    {
...
export async function startWorker() {
  ...
  try {
    ...
    console.log(
      '[Worker] Off-ramp worker started and listening to "process-offramp" queue...',
    );
  } catch (error) {
    // Do not crash the whole service on worker bootstrap errors.
    console.error("[Worker] Failed to start worker:", error);
    console.error(
      "[Worker] Check UPSTASH_REDIS_URL and UPSTASH_REDIS_TOKEN in Railway service variables.",
    );
  }
}
