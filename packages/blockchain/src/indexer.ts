...
const offrampQueue = new Queue('process-offramp', { connection });
...
async function syncEvents() {
    ...
    try {
        ...
        for (const event of eventLogs) {
            ...
            if (eventName === 'CollateralDeposited' || eventName === 'MUSDBorrowed' || eventName === 'MUSDRepaid') {
            ...
            if (eventName === 'RemittanceSent') {
                ...
                if (recipient && user) {
                    ...
                    await offrampQueue.add('offramp', {
                        txId: tx.id,
